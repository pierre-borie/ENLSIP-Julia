...
\date{8 Juin 2020 \mfabian{--} 11 Septembre 2020}
\maketitle
...
On dispose de $m$ observations réelles  $(t_{i},y_{i})$ et l'on souhaite ajuster un modèle $h$ de paramètre\mfabian{s} $x\in \mathbb{R}^{n}$ qui approche au mieux nos observations, le tout en satisfaisant $l$ contraintes, dont $q$ sont des contraintes d'égalité. Pour cela, on cherche le \mfabian{vecteur de} paramètre\mfabian{s} $x^{*}$ qui minimise la somme des écarts entre prédictions et observations au carré, soit:
\[
...
On a $r =(r_{1}, \ldots,r_{m})^{T}: \mathbb{R}^{n} \mapsto \mathbb{R}^{m}$ la multi-fonction vectorielle\fabian{répétition: une multi-fonction est une fonction vectorielle} des résidus et $c = (c_{1}, \ldots, c_{l})^{T}: \mathbb{R}^{n} \mapsto \mathbb{R}^{l}$ \mfabian{la} multi-fonction des contraintes dont les $q$ premières composantes sont des contraintes d'égalité et les $l-q$ suivantes sont des contraintes d'inégalité. On suppose les $r_{i}$ et $c_{j}$ deux fois continûment différentiables.  
\mfabian{Finalement, } $f:x\mapsto \dfrac{1}{2}\|r(x)\|^{2}$ est la fonction objectif à minimiser.
...
\newpage
\begin{algorithmic}
\REQUIRE{$x_{0}$ point de départ, $r$ et $c$ multi-fonctions des contraintes et leurs \mfabian{matrices} jacobiennes respectives $J$ et $A$}
\STATE{\textbf{begin}}
...
\subsection{Méthode de Gauss-Newton}\footnote{Fonction SUBDIR du fichier dblreduns.f} \pierre{Pour les références aux fonctions Fortran, est-ce mieux de les faire en début de partie ou en note de bas de page comme ci-dessus ?}\fabian{Personnellement, je préfère les références dans le texte plutôt qu'en notes de bas de page.}\label{method:gn}
...
Les explications ci-dessous se basent sur celles fournies \mfabian{par} \citet{lindstromwedin1988}.

Soit $\tilde{x}$ une approximation de la solution du problème \mfabian{\eqref{pb}}\fabian{Utilise eqref pour référencer les équations.}. Dans un voisinage $\delta x$\fabian{$\delta x$ est pris ici dans un voisinage, mais n'est pas lui-même un voisinage.}, à partir du développement de Taylor d'ordre 1, on a, pour $i=1,\ldots,m$, l'approximation suivante du $i$-ème résidu:\fabian{Suis-tu la notation de 
\citet{lindstromwedin1988}? Ce qui me gêne est que la notation correspond à des vecteurs lignes alors qu'il est plus standard d'avoir des vecteurs colonnes.}
\begin{equation}
...
Ce qui, en notant $J$ et $A$ les matrices jacobiennes respectives des multi-fonctions $r$ et $c$, donne les linéarisations:\fabian{Les deux équations qui suivent, elles, supposent de travailler avec des vecteurs colonnes.}
\begin{eqnarray} 
...
\end{eqnarray}
Ces linéarisations seront injectées dans la formulation du problème \eqref{pb} mais avant, nous allons nous intéresser \mfabian{au caractère} des contraintes.
...
Bien que \mfabian{\cite{lindstromwedin1988} ne mentionnent} pas précisément les hypothèses de qualification des contraintes, il m'a semblé implicite qu'une contrainte d'inégalité était considérée comme active si elle égale à $0$ et inactive si elle est strictement positive.\fabian{Il y a un peu confusion ici. La définition de contrainte active que tu donnes ici est correcte, mais les hypothèses de qualification de contraintes, comme la LICQ, servent elles à permettent d'utiliser les conditions KKT comme conditions nécessaires. L'autre point à considérer est qu'une contrainte d'inégalité active deviendra inactive si la direction de plus forte pente est réalisable.} La méthode implémentée dans ENLSIP est une méthode d'ensemble actif. Le principe est de ne prendre en compte que des contraintes actives pour les calculs de direction de descente et de pas à une itération donnée. Les contraintes d'inégalité actives peuvent être considérées comme des contraintes d'égalité. Cela permet de ramener le problème initial à un problème sous contraintes d'égalités. On pourrait penser qu'il suffit de prendre les inégalités qui sont égales à $0$ et de mettre de côté les autres. Or ce n'est pas ce qui est fait dans l'algorithme. L'idée est de travailler avec un ensemble de contraintes $\mathcal{W}$ appelé ensemble de travail (working set en anglais) mis à jour à chaque début et fin d'itération. $\mathcal{W}$ comprend toujours toutes les contraintes d'égalité.\fabian{Je crois de plus en plus que l'ensemble de travail considère les contraintes qui vont rester actives le long de la direction de recherche -- du moins leur linéarisation -- mais pas les autres; cf note précédente.}
En début d'itération, $\mathcal{W}$ contient également les contraintes d'inégalité actives au point courant\fabian{Toutes les contraintes d'inégalité actives ou seulement celles qui le resteront?}. Sa mise à jour consiste à lui retirer au plus une contrainte. Une fois les directions de descente et pas calculés, on ajoute à $\mathcal{W}$ les contraintes d'inégalité devenues actives au nouveau point courant. 
\pierre{Je pense finalement avoir compris la même chose. La contrainte que l'on retire de l'ensemble de travail est celle qui va devenir le "plus inactive" tandis que les autres vont le rester.}\fabian{À indiquer dans le texte alors.}
...
\item
les contraintes d'inégalité actives\fabian{Il faudrait peut-être trouver un autre terme ici.} considérées comme des contraintes d'égalité. 
\end{itemize}
...
À l'itération $k$,  le point courant $x_{k}$, les vecteurs $r(x_{k})$ et $\hc(x_{k})$ ainsi que les matrices $J_{k}=J(x_{k}) \text{ et } \ha_{k}=\ha(x_{k})$ sont fixés. On cherche alors la direction de descente $p_{k}$ qui nous rapproche au mieux du minimum de la fonction objectif\fabian{Pas tout à fait; il faut au moins parler du minimum contraint. Mieux: on cherche à résoudre le système KKT.}. En injectant les linéarisations de $r$ et $\hc$, on obtient le sous-problème suivant:\fabian{Utilise l'environnement align, pas array.}
...
\end{aligned}
\end{equation}
Il s'agit d'un problème de moindres carrés linéaires sous contraintes d'égalité linéaires.
...
Par transposition et par le fait que $P_{a}P_{a}^{T} = I_{t}$\fabian{Plus simplement, une matrice de permutation est elle-même orthogonale, et la transposée d'une matrice orthogonale est égale à son inverse.}, on obtient:
\begin{equation}
\ha_{k} = P_{a}
\begin{pmatrix}
L_{a} & 0
...
\mfabian{Injectant cette factorisation dans \eqref{souspb}, nous obtenons :}%l'équation des contraintes du problème \ref{souspb}: 
\[
%\ha_{k}p = -c(x_{k}) \Longleftrightarrow
P_{a}\begin{pmatrix}
L_{a} & 0
\end{pmatrix}
Q_{a}^{T}p = -c(x_{k})
...
J_{1} & J_{2}
\end{pmatrix}$, $J_{1} \in \mathcal{M}_{m\times t}$ et $J_{2} \in \mathcal{M}_{m\times (n-t)}$, puisque $Jp=JQ_{a}Q_{a}^{T}p$.\fabian{Ne sépare pas les équations de lignes blanches.}\fabian{La matrice de permutation semble avoir disparu.}
...
Si $\bar{t}<t$, autrement dit si la matrice $\ha_{k}$ est de rang déficient, il faut réaliser ce qui est appelé dans \cite{lindstromwedin1988} une stabilisation \pierre{la justification me semble d'ordre numérique mais il n y a pas d'information supplémentaire dans l'article}\fabian{Les problèmes d'une matrice de rang déficient sont connus, aussi il est concevable que les auteurs ne justifient pas davantage. Toutes les méthodes passant par les conditions KKT ont besoin de $A$ de rang plein pour que le système linéaire correspond ait une et une seule solution.}. Cela passe par la factorisation QR de $L_{a}$: 
\begin{equation} \label{qrL}
L_{a}P_{l} = Q_{l} R_{l}
\end{equation}\pierre{indice $l$ pour faire référence à la matrice triangulaire inférieure dont c'est la décomposition mais coïncide avec $l$ le nombre total de contraintes du problèmes (voir \eqref{pb})}\fabian{Il est alors possible d'utiliser l'indice $\ell$.}
...
\fabian{Un peu difficile à suivre. On avait à la page précédente $L_ap_1 = b$ ce qui ne donne pas l'équation ci-dessous si on remplace $L_a$ en utilisant \eqref{qrL}.} 
Pour $\omega_{1} \leq t$, on définit $R_l^{(\omega_{1})}$ comme \mfabian{la sous-matrice de $R_l$ constituée des éléments $r_{ij}$, avec $i \leq i,j \leq \omega_1$.}
% le bloc triangulaire supérieur composé des $\omega_{1}$ premières lignes et colonnes de $R_l$ et $b_{1}^{(\omega_{1})}$ le vecteur des $\omega_{1}$ premiers éléments de $b_{1}$.
 Par suite, posant $\delta p_1^{(\omega_{1})}$ solution du système d'inconnue $y \in \real^{\omega_{1}}$  $R_ly=b_{1}^{(\omega_{1})}$, on obtient:
...
\fabian{J'ai fait quelques ajustements dans la bibliographie, en particulier sur l'écriture des numéros de pages.}
