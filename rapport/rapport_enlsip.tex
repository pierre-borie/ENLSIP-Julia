...
\newpage
\begin{algorithmic}
\REQUIRE{$x_{0}$ point de départ, $r$ et $c$ multi-fonctions des contraintes et leurs jacobiennes respectives $J$ et $A$}
\STATE{\textbf{begin}}
...
\subsection{Méthode de Gauss-Newton}\footnote{Fonction SUBDIR du fichier dblreduns.f} \pierre{Pour les références aux fonctions Fortran, est-ce mieux de les faire en début de partie ou en note de bas de page comme ci-dessus ?}\label{method:gn}
...
\item
les contraintes d'inégalité actives considérées comme des contraintes d'égalité. 
\end{itemize}
