...
J_{1} & J_{2}
\end{pmatrix}$, $J_{1} \in \mathcal{M}_{m\times t}$ et $J_{2} \in \mathcal{M}_{m\times (n-t)}$, puisque $Jp=JQ_{a}Q_{a}^{T}p$, $Q_{a}$ étant orthogonale.
...
Si $\bar{t}<t$, autrement dit si la matrice $\ha_{k}$ est de rang déficient, il faut réaliser ce qui est appelé dans \cite{lindstromwedin1988} une stabilisation. Cela passe par la factorisation QR de $L_{a}$: 
...
\Longleftrightarrow& R_{\ell}P_{\ell}^Tp_1 = b_{1}
\end{aligned}
$$
...
\pierre{J'ai pourtant repris les mêmes équations que celle de l'article en modifiant seulement les indices...}
Pour $\omega_{1} \leq t$, on définit $R_{\ell}^{(\omega_{1})}$ comme la sous-matrice de $R_{\ell}$ constituée des éléments $r_{ij}$, avec $i \leq \omega_{1},j \leq \omega_1$. $b_{1}^{(\omega_{1})}$ désigne le vecteur constitué des $\omega_{1}$ premiers éléments de $b_{1}$.
...
Ru=v
\end{equation}
Cela définit comment découper la matrice $R$ afin de résoudre ce système comme en \eqref{calculp1} ou \eqref{calculp2}.
...
\begin{enumerate}
\item
Si $z^{T}\hat{w}^{(old)} \geq \mu$ et $\omega_{1}\neq t$ ($\omega_{1}$ est la dimension définie en \eqref{calculp1})
